...
var beta = r0/durInf; // infectiousness
var gamma = 1.0/durInf; // recovery
var mu = 1.0/durImm; // loss of immunity
...
var sir_color = {S: "#440154FF", I: "#21908CFF", R: "#FDE725FF" }
...
$("#durInf").val(durInf)
$("#durInf").keyup(update_durInf);

$("#durImm").val(durImm)
$("#durImm").keyup(update_durImm);
...
setInterval(run_SIR, 100);

function run_SIR() {

    // when no more I nodes stop the simulation
    if(running == 0){
      return;
    }

    // SI nodes
    var SInode = []; // nodes in contact
    var SIcount = 0; // number of SI connections
    for(i in nodeArray){
      if(nodeArray[i].state == "I"){
        ix = parseInt(nodeArray[i].name);
        for(j in edgeArray){
          if(edgeArray[j].source == ix && nodeArray[edgeArray[j].target].state == "S"){
            SInode.push({
              S:ix, I: parseInt(nodeArray[edgeArray[j].target].name)
            });
            SIcount++;
          }
        }
    ...
    for(i in SInode){
      console.log("si node:");
      console.log(SInode[i].S);
      console.log(SInode[i].I);
    }
    ...
    for(i in nodeArray){
      if(nodeArray[i].state == "I"){
        Inode.push(i);
        console.log("HELLO!!!!!!!!!sadfasdfasdfas");
      }
      if(nodeArray[i].state == "R"){
        Rnode.push(i);
        console.log("HELLO!!!!!!!!!sadfasdfasdfas");
      }
    }

    var Beta = beta * SIcount;
    var Lambda = Beta + gamma + mu; // cumulative hazard of anything happening

    // draw our two random numbers (Gillespie's First Reaction method)
    var r1 = Math.random();
    var r2 = Math.random();

    var tau = (1.0/Lambda) * Math.log(1.0/r1);// whats the next time something happens? (uses up one random number)
    time += tau;
    ...
    // S -> I
    if(z < Beta){
      // since we're on a network we need one more random number =[
      var ix = Math.floor(Math.random() * SIcount);
      nodeArray[SInode[ix].S].state = "I";
      epi_state.I ++;
      epi_state.S --;
    // I -> R
    } else if(Beta <= z < Beta + gamma){
      var ix = Math.floor(Math.random() * Inode.length);
      var ixNode = Inode[ix];
      nodeArray[ixNode].state = "R";
      epi_state.I --;
      epi_state.R ++;
    // R -> S
    } else if(Beta + gamma <= z){
      var ix = Math.floor(Math.random() * Rnode.length);
      var ixNode = Rnode[ix];
      nodeArray[ixNode].state = "S";
      epi_state.R --;
      epi_state.S ++;
    }
...
function update_r0 () {
  ...
  if (isNaN(p) || p<0.0) {
     valid_data = 0;
     $("#r0").css("background-color", "#f88");
  } else {
     r0 = p;
     valid_data = 1;
     $("#r0").css("background-color", "#fff");
  }
}

function update_durInf () {
  p = Number($("#durInf").val());
  if (isNaN(p) || p<0.0) {
     valid_data = 0;
     $("#durInf").css("background-color", "#f88");
  } else {
     durInf = p;
     valid_data = 1;
     $("#durInf").css("background-color", "#fff");
  }
}

function update_durImm () {
  p = Number($("#durImm").val());
  if (isNaN(p) || p<0.0) {
     valid_data = 0;
     $("#durImm").css("background-color", "#f88");
  } else {
     durImm = p;
     valid_data = 1;
     $("#durImm").css("background-color", "#fff");
  }
}
